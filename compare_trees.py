...
class Forest:
    ...
    def draw_a_tree(self, TOP_K_time_consuming = TOP_K_TIME_CONSUMING, FILTER_lonelyRoot = FILTER_LONELYROOT, \
    FILTER_noRequest = FILTER_NOREQUEST):
        print "Tree list:"
        ...
        if ind == (1+len(self.forest)) and self.intersec_tree!=None: #draw critical path
            self.painter.draw_tree( self.intersec_tree, TOP_K_time_consuming, FILTER_lonelyRoot, FILTER_noRequest)
            return True
        if ind<0 or ind> (1+len(self.forest)):
            print "invalid input"
            return True
        if ind < len(self.forest) and ind>=0:
            self.painter.draw_tree( self.forest[ind], TOP_K_time_consuming, FILTER_lonelyRoot, FILTER_noRequest)
        else:
            self.draw_all()
        return True
